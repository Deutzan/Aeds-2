...
//-- getters e setters --//
   ...
   public static void Insertsort(Veiculo[] veiculos) {

    // começa na segunda posição
    for (int i = 1; i < veiculos.length && veiculos[i] != null; i++) {
        Veiculo atual = veiculos[i];
        int j = i - 1;

        // enquanto o anterior vier depois alfabeticamente
        while (j >= 0 && veiculos[j].getMarca().compareTo(atual.getMarca()) > 0) {

            // desloca o veículo anterior para a direita
            veiculos[j + 1] = veiculos[j];
            j--;
        }

        // coloca o atual na posição correta
        veiculos[j + 1] = atual;
    }
}
}